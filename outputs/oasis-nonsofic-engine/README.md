...
The certificate has closed the formerly vague spectral-constant and Thompson-`V` inputs. Three proof-wide quantitative obligations remain: an effective radius for Kun's ultraproduct locality lemma, a finite expander-decomposition bound at that radius, and the resulting finite-word threshold sufficient for one explicit obstruction. These fields remain explicit and are never silently filled with guessed numbers.
